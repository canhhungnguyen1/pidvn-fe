...
export class BalanceSheetComponent implements OnInit, AfterViewInit {
  ...
  constructor(protected service: PurchaseDataService, private toastr: ToastrService) {
    //this.genPivotTable([]);
  }
  ...
  ngOnInit(): void {
    ...
    var toDate = new Date(curD.getFullYear(), curD.getMonth() + 1, 0);
  ...
  getBalanceSheet() {
    this.service.getBalanceSheet(this.searchVo.factory, this.searchVo.date).subscribe(
      response => {
